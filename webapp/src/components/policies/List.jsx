...
const PoliciesList = () => {
  ...
  const filteredData = policiesData.filter(
    (policy) =>
      policy.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      policy.scanner.toLowerCase().includes(searchQuery.toLowerCase())
  );
    ...
    <div className="bg-gray-100 p-6">
      <div className="container mx-auto">
        ...
        <div className="mb-4">
          <input
            type="text"
            placeholder="Search by Name or Scanner..."
            value={searchQuery}
          ...
          <table className="min-w-full bg-white border border-gray-300 rounded-lg shadow-md">
            <thead>
              <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                ...
                <th
                  className="py-3 px-6 text-left cursor-pointer"
                  onClick={() => requestSort("scanner")}
                >
                  Scanner {getSortingArrow("scanner")}
                </th>
              </tr>
            </thead>
            <tbody className="text-gray-600 text-sm font-light">
              {currentItems.map((policy) => (
                ...
                >
                  ...
                  <td className="py-3 px-6">{policy.scanner}</td>
                </tr>
              ))}
            </tbody>
          </table>
